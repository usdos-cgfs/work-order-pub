...
`+O.get_stackTrace()),t(O)}let b={oUser:a,resolve:e,reject:t};o.load(a),o.executeQueryAsync(Function.createDelegate(b,d),Function.createDelegate(b,m))})}function fa(s){for(var e=new Array,t=s.getEnumerator();t.moveNext();){var i=t.get_current();e.push(qi(i))}return e}N.getSPSiteGroupByName=function(s){var e=null;return this.globalConfig.siteGroups!=null&&(e=this.globalConfig.siteGroups.find(function(t){return t.Title==s})),e};function Ni(s){var e=this;e.config={def:s};async function t(){if(!e.config.fieldSchema){let r=`/web/lists/GetByTitle('${e.config.def.title}')/Fields`,n=await ce(r);e.config.fieldSchema=n.d.results}}t();function i(r,n){return new Promise((c,u)=>{o(r,c,n)})}function o(r,n,c){c=c===void 0?!1:c;var u=new Array,f=new Array,C=new SP.ClientContext.get_current,g=C.get_web(),p=g.get_lists().getByTitle(e.config.def.title);r.forEach(function(A){var R=N.getSPSiteGroupByName(A[0]);R?f.push([R,A[1]]):u.push([C.get_web().ensureUser(A[0]),A[1]])});function q(){console.log("Successfully found item");var A=new SP.ClientContext.get_current,R=A.get_web();c?(p.resetRoleInheritance(),p.breakRoleInheritance(!1,!1),p.get_roleAssignments().getByPrincipal(N.globalConfig.currentUser).deleteObject()):p.breakRoleInheritance(!1,!1),this.resolvedGroups.forEach(function(G){var j=SP.RoleDefinitionBindingCollection.newObject(A);j.add(R.get_roleDefinitions().getByName(G[1])),p.get_roleAssignments().add(G[0],j)}),this.users.forEach(function(G){var j=SP.RoleDefinitionBindingCollection.newObject(A);j.add(R.get_roleDefinitions().getByName(G[1])),p.get_roleAssignments().add(G[0],j)});var S={oList:p,callback:n};function D(){console.log("Successfully set permissions"),n(p)}function L(G,j){console.error("Failed to update permissions on List: "+this.oList.get_title()+j.get_message()+`
`,j.get_stackTrace())}A.load(p),A.executeQueryAsync(Function.createDelegate(S,D),Function.createDelegate(S,L))}function v(A,R){console.error("Failed to find List: "+this.oList.get_title+R.get_message(),R.get_stackTrace())}var w={oList:p,users:u,resolvedGroups:f,callback:n};C.load(p),u.map(function(A){C.load(A[0])}),C.executeQueryAsync(Function.createDelegate(w,q),Function.createDelegate(w,v))}function a(r){return r&&(Array.isArray(r)?r.map(n=>d(n)).join(";#"):d(r))}function d(r){return r.ID?`${r.ID};#${r.LookupValue??""}`:r.LookupValue?r.LookupValue:r.constructor.getName()=="Date"?r.toISOString():r}function m(r,n=null){return new Promise((c,u)=>{let f=new SP.ClientContext.get_current,g=f.get_web().get_lists().getByTitle(e.config.def.title),p=new SP.ListItemCreationInformation;if(n){var q=N.globalConfig.siteUrl+"/Lists/"+e.config.def.name+"/"+n;p.set_folderUrl(q)}let v=g.addItem(p),w=["ID","Author","Created","Editor","Modified"];Object.keys(r).filter(D=>!w.includes(D)).forEach(D=>{v.set_item(D,a(r[D]))}),v.update();function A(){c(v.get_id())}function R(D,L){console.error("Create Item Failed - List: "+e.config.def.name),console.error("ValuePairs",r),console.error(D,L),u(D)}let S={entity:r,oListItem:v,resolve:c,reject:u};f.load(v),f.executeQueryAsync(Function.createDelegate(S,A),Function.createDelegate(S,R))})}function b(r){if(!r)return r;let n={};switch(r.constructor.getName()){case"SP.FieldUserValue":n.LoginName=r.get_email();case"SP.FieldLookupValue":n.ID=r.get_lookupId(),n.LookupValue=r.get_lookupValue(),n.Title=r.get_lookupValue();break;default:n=r}return n}function x(r,n,c){var u=new SP.CamlQuery.createAllItemsQuery;u.set_viewXml(r);var f=new SP.ClientContext.get_current,C=f.get_web(),g=C.get_lists().getByTitle(e.config.def.title),p=g.getItems(u);function q(){var A=this,R=A.collListItem.getEnumerator();let S=[];for(;R.moveNext();){var D=R.get_current(),L={};n.forEach(G=>{var j=D.get_item(G);L[G]=Array.isArray(j)?j.map(fe=>b(fe)):b(j)}),S.push(L)}c(S)}function v(A,R){console.log("unsuccessful read",A),alert("Request on list "+e.config.def.name+` failed, producing the following error: 
 `+R.get_message()+`
StackTrack: 
 `+R.get_stackTrace())}var w={collListItem:p,callback:c,fields:n,camlQuery:u};f.load(p,`Include(${n.join(", ")})`),f.executeQueryAsync(Function.createDelegate(w,q),Function.createDelegate(w,v))}function O({fields:r=null,caml:n=null}){if(!n)var n='<View Scope="RecursiveAll"><Query><Where><Eq><FieldRef Name="FSObjType"/><Value Type="int">0</Value></Eq></Where></Query></View>';return new Promise((c,u)=>{x(n,r,c)})}function _(r,n){return new Promise((c,u)=>{try{U(r,n,c)}catch(f){u(f)}})}async function J(r,n){let[c,u]=await Z(n),f=`/web/lists/GetByTitle('${e.config.def.title}')/items(${r})?$Select=${c}&$expand=${u}`;return(await ce(f)).d}async function ye(){if(!e.config.fieldSchema){let r=`/web/lists/GetByTitle('${e.config.def.title}')/Fields`,n=await ce(r);e.config.fieldSchema=n.d.results}return e.config.fieldSchema}async function Z(r){let n=[],c=[],u=await ye();return r.map(f=>{if(f.includes("/")){n.push(f),c.push(f.split("/")[0]);return}let C=u.find(g=>g.StaticName==f);if(!C){alert(`Field '${f}' not found on list ${e.config.def.name}`);return}switch(C.TypeAsString){case"UserMulti":case"User":case"LookupMulti":case"Lookup":let g=f+"/ID",p=f+"/Title";n.push(g),n.push(p),c.push(f);break;case"Choice":default:n.push(f)}}),[n,c]}async function oe(r,{orderByColumn:n=null,sortAsc:c},{count:u=null},f,C){let[g,p]=await Z(f),q=n?`$orderby=${n} ${c?"asc":"desc"}`:"",v=[];r.forEach(j=>v.push(`${j.column} ${j.op??"eq"} '${j.value}'`)),C||v.push("FSObjType eq '0'");let w="$filter=("+v.join(") and (")+")",A="$select="+g,R="$expand="+p,S=u?`$top=${u}`:"",D=`/web/lists/GetByTitle('${e.config.def.title}')/items?${A}&${R}&${q}&${w}&${S}`,L=await ce(D);return{results:L?.d?.results,_next:L?.d?.__next}}async function he(r){let n=await ce(r._next);return{results:n?.d?.results,_next:n?.d?.__next}}function U(r,n,c){var u=new SP.ClientContext.get_current,f=u.get_web(),C=f.get_lists().getByTitle(e.config.def.title),g=C.getItemById(r);function p(){let w={};n.forEach(A=>{var R=g.get_item(A);w[A]=Array.isArray(R)?R.map(S=>b(S)):b(R)}),c(w)}function q(w,A){console.error("SAL: findById - List: "+e.config.def.name),console.error("Fields",this),console.error(w,A)}var v={oListItem:g,callback:c,fields:n};u.load(g),u.executeQueryAsync(Function.createDelegate(v,p),Function.createDelegate(v,q))}function ae(r){return r?.ID?new Promise((n,c)=>{let u=new SP.ClientContext.get_current,g=u.get_web().get_lists().getByTitle(e.config.def.title).getItemById(r.ID),p=["ID","Author","Created","Editor","Modified"];Object.keys(r).filter(A=>!p.includes(A)).forEach(A=>{g.set_item(A,a(r[A]))}),g.update();function q(){console.log("Successfully updated "+this.oListItem.get_item("Title")),n()}function v(A,R){console.error("Update Failed - List: "+e.config.def.name),console.error("Item Id",this.oListItem.get_id()??"N/A"),console.error(r),console.error(A,R),c(R)}let w={oListItem:g,entity:r,resolve:n,reject:c};u.load(g),u.executeQueryAsync(Function.createDelegate(w,q),Function.createDelegate(w,v))}):!1}function tt(r,n){var c=new SP.ClientContext.get_current,u=c.get_web(),f=u.get_lists().getByTitle(e.config.def.title),C={callback:n};f.getItemById(r).deleteObject();function p(v,w){n()}function q(v,w){console.error("sal.SPList.deleteListItem: Request on list "+e.config.def.name+` failed, producing the following error: 
 `+w.get_message()+`
StackTrack: 
 `+w.get_stackTrace())}c.executeQueryAsync(Function.createDelegate(C,p),Function.createDelegate(C,q))}function zt(r){return new Promise((n,c)=>tt(r,n))}function _o(r,n,c){return new Promise((u,f)=>{Bo(r,n,u,c)})}function Bo(r,n,c,u){u=u===void 0?!1:u;let f=[],C=[],g=new SP.ClientContext.get_current,v=g.get_web().get_lists().getByTitle(e.config.def.title).getItemById(r);n.forEach(function(S){let D=N.getSPSiteGroupByName(S[0]);D?.oGroup?C.push([D.oGroup,S[1]]):f.push([g.get_web().ensureUser(S[0]),S[1]])});function w(){console.log("Successfully found item");let S=new SP.ClientContext.get_current,D=S.get_web();u?(v.resetRoleInheritance(),v.breakRoleInheritance(!1,!1),v.get_roleAssignments().getByPrincipal(N.globalConfig.currentUser).deleteObject()):v.breakRoleInheritance(!1,!1),this.resolvedGroups.forEach(function(fe){let re=SP.RoleDefinitionBindingCollection.newObject(S);re.add(D.get_roleDefinitions().getByName(fe[1])),v.get_roleAssignments().add(fe[0],re)}),this.users.forEach(function(fe){let re=SP.RoleDefinitionBindingCollection.newObject(S);re.add(D.get_roleDefinitions().getByName(fe[1])),v.get_roleAssignments().add(fe[0],re)});var L={oListItem:v,callback:c};function G(){console.log("Successfully set permissions"),c(v)}function j(fe,re){console.error("Failed to update permissions on item: "+this.oListItem.get_lookupValue()+re.get_message()+`
`+re.get_stackTrace(),!1)}S.load(v),S.executeQueryAsync(Function.createDelegate(L,G),Function.createDelegate(L,j))}function A(S,D){console.error("Failed to update permissions on item: "+this.title+D.get_message()+`
`+D.get_stackTrace(),!1)}var R={id:r,oListItem:v,users:f,resolvedGroups:C,callback:c};g.load(v),f.map(function(S){g.load(S[0])}),g.executeQueryAsync(Function.createDelegate(R,w),Function.createDelegate(R,A))}function Uo(r){return new Promise((n,c)=>{var u=new SP.ClientContext.get_current,f=u.get_web(),C=f.get_lists().getByTitle(e.config.def.title),g=C.getItemById(r),p=g.get_roleAssignments();function q(){for(var A=new SP.ClientContext.get_current,R=A.get_web(),S=[],D=this.roles.getEnumerator();D.moveNext();){var L=D.get_current(),G=L.get_member();A.load(G),S.push(G)}A.executeQueryAsync(function(j,fe){var re=S.map(function(Ce){return{ID:Ce.get_id(),Title:Ce.get_title(),LoginName:Ce.get_loginName()}});n(re)},function(j,fe){alert("Request failed. "+fe.get_message()+`
`+fe.get_stackTrace()),c(fe)})}function v(A,R){console.error("Failed to get permissions on item: "+R.get_message()+`
`+R.get_stackTrace(),!1),c(R)}let w={id:r,oListItem:g,roles:p,resolve:n,reject:c};u.load(g),u.load(p),u.executeQueryAsync(Function.createDelegate(w,q),Function.createDelegate(w,v))})}function Oe(r){let n=e.config.def.isLib?"/"+e.config.def.name+"/":"/Lists/"+e.config.def.name+"/";return N.globalConfig.siteUrl+n+r}function Go(r){return e.config.def.isLib?new Promise((n,c)=>Ko(r,n)):new Promise((n,c)=>zo(r,n))}async function $o(r){try{let c=(await Qo(r)).map(u=>[u.LoginName,N.config.siteRoles.roles.RestrictedRead]);await Jt(r,c,!0)}catch(n){console.warn(n)}}async function jo(r,n){let c=Oe(r),u=N.globalConfig.siteUrl+`/_api/web/GetFolderByServerRelativeUrl('${c}')/ListItemAllFields/RoleAssignments?$expand=Member,Member/Users,RoleDefinitionBindings`,f=await fetch(u,{method:"GET",headers:{Accept:"application/json; odata=verbose"}});if(!f.ok){if(f.status==404)return;console.error(f)}let C=await f.json(),g=C?.d?.results;if(!g){console.warn("No results found",C);return}let p=n.filter(q=>{let v=q[0],w=q[1];return!g.find(R=>R.Member.LoginName!=v&&!R.Member.Users?.results.find(S=>S.LoginName==v)?!1:!!R.RoleDefinitionBindings.results?.find(S=>N.config.siteRoles.fulfillsRole(S.Name,w)))});console.log("Adding missing permissions",p),p.length&&await Jt(r,p,!1)}function Ho(r,n){return new Promise((c,u)=>{let f=new SP.ClientContext.get_current,g=f.get_web().get_lists().getByTitle(e.config.def.title),p=Oe(r),q=SP.CamlQuery.createAllItemsQuery();q.set_folderServerRelativeUrl(p);let v=g.getItems(q);f.load(v,`Include(${n.join(", ")})`),f.executeQueryAsync(function(){let w=[];for(var A=v.getEnumerator();A.moveNext();){var R=A.get_current(),S={};n.forEach(D=>{var L=R.get_item(D);S[D]=Array.isArray(L)?L.map(G=>b(G)):b(L)}),S.oListItem=R,w.push(S)}c(w)},function(w,A){console.warn("Unable load list folder contents:"),console.error(w),console.error(A),u(A)})})}async function Qo(r){return new Promise(async(n,c)=>{let u=await Wo(r);if(!u){c("Folder item does not exist");return}let f=u.get_roleAssignments(),C=new SP.ClientContext.get_current;C.load(u),C.load(f),C.executeQueryAsync(function(){let g=new SP.ClientContext.get_current;console.log(u);let p=[],q=[],v=f.getEnumerator();for(;v.moveNext();){let w=v.get_current(),A=w.get_member(),R=w.get_roleDefinitionBindings();g.load(R),g.load(A),p.push({principal:A,bindings:R})}g.executeQueryAsync(function(w,A){let R=p.map(function({principal:S,bindings:D}){let L=[],G=D.getEnumerator();for(;G.moveNext();){let j=G.get_current();L.push(j.get_name())}return{ID:S.get_id(),Title:S.get_title(),LoginName:S.get_loginName(),Roles:L}});n(R)},function(w,A){console.warn("Unable load folder principals permissions:"),console.error(w),console.error(A),c(A)})},function(g,p){console.warn("Unable load folder permissions:"),console.error(g),console.error(p),c(p)})})}async function Wo(r){return new Promise((n,c)=>{let u=new SP.ClientContext.get_current,C=u.get_web().get_lists().getByTitle(e.config.def.title),g=SP.CamlQuery.createAllItemsQuery();var q='<View Scope="RecursiveAll"><Query><Where><And><Eq><FieldRef Name="FSObjType"/><Value Type="int">1</Value></Eq><Eq><FieldRef Name="FileRef"/><Value Type="Text">'+Oe(r)+"</Value></Eq></And></Where></Query><RowLimit>1</RowLimit></View>";g.set_viewXml(q);let v=C.getItems(g);async function w(){let S=[];for(var D=v.getEnumerator();D.moveNext();){let G=D.get_current();S.push(G)}S||(console.warn("folder not found"),n(S)),S.length>1&&(console.warn("Multiple folders found!"),n(S));let L=S[0];n(L)}function A(S,D){console.warn("Unable load list folder contents:"),console.error(S),console.error(D),c(D)}let R={allFolders:v,resolve:n,reject:c};u.load(v),u.executeQueryAsync(Function.createDelegate(R,w),Function.createDelegate(R,A))})}function zo(r,n){var c=r.split("/"),u=0,f=function(C,g,p,q){var v=g[p];p++;var w=g.slice(0,p).join("/");Jo(w,function(A){p>=g.length?q(A.get_id()):f(w,g,p,q)},function(){e.createListFolder(v,function(A){p>=g.length?q(A):f(w,g,p,q)},C)})};f("",c,u,n)}e.createListFolder=function(r,n,c){c=c===void 0?"":c;let u=new SP.ClientContext.get_current,C=u.get_web().get_lists().getByTitle(e.config.def.title),g="",p=new SP.ListItemCreationInformation;p.set_underlyingObjectType(SP.FileSystemObjectType.folder),p.set_leafName(r),c&&(g=N.globalConfig.siteUrl+"/Lists/"+e.config.def.name+"/"+c,p.set_folderUrl(g));let q=C.addItem(p);q.set_item("Title",r),q.update();function v(R,S){n(this.newItem.get_id())}function w(R,S){alert("Request on list "+e.config.def.name+` failed, producing the following error: 
`+S.get_message()+`
StackTrack: 
`+S.get_stackTrace())}let A={folderName:r,callback:n,newItem:q};u.load(q),u.executeQueryAsync(Function.createDelegate(A,v),Function.createDelegate(A,w))};function Jo(r,n,c){var u=N.globalConfig.siteUrl+"/Lists/"+e.config.def.name+"/"+r,f=SP.ClientContext.get_current(),C=f.get_web().getFolderByServerRelativeUrl(u);C.get_listItemAllFields();var g={folder:C,path:r,onExists:n,onNonExists:c};f.load(C,"Exists","Name");function p(){if(C.get_exists()){let A=function(){n(w)},R=function(S,D){console.error("Failed to find folder at "+r,D)};console.log("Folder "+C.get_name()+" exists in "+e.config.def.name);var v=new SP.ClientContext.get_current,w=C.get_listItemAllFields();g={folderItem:w,path:r,onExists:n},v.load(w),v.executeQueryAsync(Function.createDelegate(g,A),Function.createDelegate(g,R))}else console.warn("Folder exists but is hidden (security-trimmed) for us.")}function q(v,w){w.get_errorTypeName()==="System.IO.FileNotFoundException"?(console.log("SAL.SPList.ensureListFolder:           Folder "+r+" does not exist in "+e.config.def.name),c()):console.error("Error: "+w.get_message())}f.executeQueryAsync(Function.createDelegate(g,p),Function.createDelegate(g,q))}function Ko(r,n){let f=new SP.ClientContext.get_current().get_web().get_lists().getByTitle(e.config.def.title);var C=function(g,p,q){var v=g.get_context(),w=p.split("/"),A=w[0],R=g.get_folders().add(A);v.load(R),v.executeQueryAsync(function(){if(w.length>1){var S=w.slice(1,w.length).join("/");C(R,S,q)}else q(R)},function(S,D){console.error("error creating new folder"),console.error(S),console.error(error)})};C(f.get_rootFolder(),r,n)}function Jt(r,n,c){return new Promise((u,f)=>{Xo(r,n,u,c)})}function Xo(r,n,c,u){u=u===void 0?!1:u;var f=[],C=[];let g=Oe(r),p=new SP.ClientContext.get_current,v=p.get_web().getFolderByServerRelativeUrl(g);n.forEach(function(S){var D=N.getSPSiteGroupByName(S[0]);D?.oGroup?C.push([D.oGroup,S[1]]):f.push([p.get_web().ensureUser(S[0]),S[1]])});function w(){var S=new SP.ClientContext.get_current,D=S.get_web(),L=this.folder.get_listItemAllFields();u?(L.resetRoleInheritance(),L.breakRoleInheritance(!1,!1),L.get_roleAssignments().getByPrincipal(N.globalConfig.currentUser).deleteObject()):L.breakRoleInheritance(!1,!1),this.resolvedGroups.forEach(function(re){var Ce=SP.RoleDefinitionBindingCollection.newObject(S);Ce.add(D.get_roleDefinitions().getByName(re[1])),L.get_roleAssignments().add(re[0],Ce)}),this.users.forEach(function(re){var Ce=SP.RoleDefinitionBindingCollection.newObject(S);Ce.add(D.get_roleDefinitions().getByName(re[1])),L.get_roleAssignments().add(re[0],Ce)});var G={folderItem:L,callback:c};function j(){console.log("Successfully set permissions"),this.callback(L)}function fe(re,Ce){console.error("Failed to update permissions on item: "+Ce.get_message(),Ce)}S.load(L),S.executeQueryAsync(Function.createDelegate(G,j),Function.createDelegate(G,fe))}function A(S,D){console.error("Something went wrong setting perms on library folder",D)}var R={folder:v,users:f,callback:c,resolvedGroups:C,valuePairs:n,reset:u};f.map(function(S){p.load(S[0])}),p.load(v),p.executeQueryAsync(Function.createDelegate(R,w),Function.createDelegate(R,A))}function Yo(r){let c=`/web/GetFolderByServerRelativeUrl('${Oe(r)}')`;return ce(c,"POST",{"If-Match":"*","X-HTTP-Method":"DELETE"})}function Zo(r,n,c,u){var f="";c.id&&(f=c.id);let C=SP.UI.$create_DialogOptions();var g=e.config.def.isLib?"/"+e.config.def.name+"/":"/Lists/"+e.config.def.name+"/",p="";c.rootFolder&&(p=N.globalConfig.siteUrl+g+c.rootFolder);var q=e.config.def.isLib?"/"+e.config.def.name+"/Forms/":"/Lists/"+e.config.def.name+"/";Object.assign(C,{title:n,dialogReturnValueCallback:u,args:JSON.stringify(c),url:N.globalConfig.siteUrl+q+r+"?ID="+f+"&Source="+location.pathname+"&RootFolder="+p}),SP.UI.ModalDialog.showModalDialog(C)}function ea(r,n,c){return new Promise((u,f)=>{let C=new SP.ClientContext.get_current,p=C.get_web().get_lists().getByTitle(e.config.def.title);C.load(p),C.executeQueryAsync(function(){var q=N.globalConfig.siteUrl=="/"?"":N.globalConfig.siteUrl;let v=SP.UI.$create_DialogOptions();Object.assign(v,{title:n,dialogReturnValueCallback:u,args:JSON.stringify(c),url:q+"/_layouts/Upload.aspx?List="+p.get_id().toString()+"&RootFolder="+q+"/"+e.config.def.name+"/"+encodeURI(r)+"&Source="+location.pathname+"&args="+encodeURI(JSON.stringify(c))}),SP.UI.ModalDialog.showModalDialog(v)},function(q,v){console.error("Error showing file modal: "),console.error(q),console.error(v)})})}let Ri=10485760,Ya={start:"startupload",continue:"continueupload",finish:"finishupload"};async function ta(r,n,c,u){let f=r,C=Ri,g=r.size,p=parseInt((g/C).toString(),10)+(g%C===0?1:0),q=n+"/"+c,v=ga(),w;u({currentBlock:0,totalBlocks:p}),w=await sa(v,r.slice(0,C),q,n);for(let R=2;R<p;R++)u({currentBlock:R,totalBlocks:p}),w=await ia(v,r.slice(w,w+C),w,q);u({currentBlock:p-1,totalBlocks:p});let A=await oa(v,r.slice(w),w,q);return u({currentBlock:p,totalBlocks:p}),A}async function sa(r,n,c,u){let f=`/web/getFolderByServerRelativeUrl(@folder)/files/getByUrlOrAddStub(@file)/StartUpload(guid'${r}')?&@folder='${u}'&@file='${c}'`,p=await ce(f,"POST",{"Content-Type":"application/octet-stream"},{body:n});if(!p){console.error("Error starting upload!");return}return parseFloat(p.d.StartUpload)}async function ia(r,n,c,u){let f=`/web/getFileByServerRelativeUrl(@file)/ContinueUpload(uploadId=guid'${r}',fileOffset=${c})?&@file='${u}'`,p=await ce(f,"POST",{"Content-Type":"application/octet-stream"},{body:n});if(!p){console.error("Error starting upload!");return}return parseFloat(p.d.ContinueUpload)}async function oa(r,n,c,u){let f=`/web/getFileByServerRelativeUrl(@file)/FinishUpload(uploadId=guid'${r}',fileOffset=${c})?&@file='${u}'`,p=await ce(f,"POST",{"Content-Type":"application/octet-stream"},{body:n});if(!p){console.error("Error starting upload!");return}return p}async function aa(r,n,c){return await fetch(_spPageContextInfo.webServerRelativeUrl+`/_api/web/GetFolderByServerRelativeUrl('${n}')/Files/add(url='${c}',overwrite=true)`,{method:"POST",credentials:"same-origin",body:r,headers:{Accept:"application/json; odata=verbose","Content-Type":"application/json;odata=nometadata","X-RequestDigest":document.getElementById("__REQUESTDIGEST").value}}).then(u=>{if(!u.ok){console.error("Error Uploading File",u);return}return u.json()})}async function ra(r,n,c,u,f=null){f||(f=()=>{});let C=Oe(c),g=null;if(r.size>Ri){let v=()=>ta(r,C,n,f);g=await ba.addJob(v)}else f({currentBlock:0,totalBlocks:1}),g=await aa(r,C,n),f({currentBlock:1,totalBlocks:1});await na(g.d,u),await ca(C+"/"+n,"");let p=g.d.ListItemAllFields.__deferred.uri+"?$select=ID";return(await ce(p)).d.ID}async function na(r,n){var c=await fetch(r.ListItemAllFields.__deferred.uri,{method:"POST",credentials:"same-origin",body:JSON.stringify(n),headers:{Accept:"application/json; odata=nometadata","Content-Type":"application/json;odata=nometadata","X-RequestDigest":document.getElementById("__REQUESTDIGEST").value,"X-HTTP-Method":"MERGE","If-Match":"*"}}).then(u=>{if(!u.ok){console.error("Error Updating File",u);return}return u});return c}function la(r,n,c,u){let f=Oe(r),C=Oe(n);var g=new SP.ClientContext.get_current,p=g.get_web(),q=p.getFolderByServerRelativeUrl(f);g.load(q,"Files"),g.executeQueryAsync(function(){for(var v=q.get_files(),w=v.getEnumerator(),A=[];w.moveNext();){var R=w.get_current(),S=C+"/"+R.get_name();A.push(S),R.copyTo(S,!0)}console.log(A),g.executeQueryAsync(function(){console.log("Files moved successfully!"),c()},function(D,L){console.log("error: ",L.getMessage()),u(L)})},function(v,w){console.error("Unable to copy files: ",w.get_message()),console.error(v,w),u(w)})}function da(r,n){return new Promise((c,u)=>{la(r,n,c,u)})}async function Za(){let r=await ce(`/web/lists/GetByTitle('${e.config.def.title}')`)}function ca(r,n){let c=`/web/GetFileByServerRelativeUrl('${r}')/CheckIn(comment='${n}',checkintype=0)`;return ce(c,"POST")}return{findByIdAsync:_,getById:J,findByColumnValueAsync:oe,loadNextPage:he,getListItemsAsync:O,createListItemAsync:m,updateListItemAsync:ae,deleteListItemAsync:zt,setItemPermissionsAsync:_o,getItemPermissionsAsync:Uo,getFolderContentsAsync:Ho,upsertFolderPathAsync:Go,deleteFolderByPathAsync:Yo,getServerRelativeFolderPath:Oe,setFolderReadonlyAsync:$o,setFolderPermissionsAsync:Jt,ensureFolderPermissionsAsync:jo,uploadFileToFolderAndUpdateMetadata:ra,uploadNewDocumentAsync:ea,copyFilesAsync:da,showModal:Zo}}async function ce(s,e="GET",t={},i={}){let o=s.startsWith("http")?s:N.globalConfig.siteUrl+"/_api"+s,a=await fetch(o,{method:e,headers:{Accept:"application/json; odata=verbose","X-RequestDigest":document.getElementById("__REQUESTDIGEST").value,...t},...i});if(!a.ok){if(a.status==404)return;console.error(a)}try{return await a.json()}catch{return}}window.spFetch=ce;function ga(){if(crypto.randomUUID)return crypto.randomUUID();let s=Date.now();return"xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,function(e){let t=(s+Math.random()*16)%16|0;return s=Math.floor(s/16),(e==="x"?t:t&3|8).toString(16)})}var Yt=class{constructor(e){this.maxConcurrency=e,this.runningJobs=0,this.queue=[]}addJob(e){return new Promise((t,i)=>{let o=async()=>{try{let a=await e();t(a)}catch(a){i(a)}finally{this.runningJobs--,this.processQueue()}};this.queue.push(o),this.processQueue()})}processQueue(){for(;this.runningJobs<this.maxConcurrency&&this.queue.length>0;){let e=this.queue.shift();this.runningJobs++,e()}}},ba=new Yt(5);var X={FullControl:"Full Control",Design:"Design",Edit:"Edit",Contribute:"Contribute",Read:"Read",LimitedAccess:"Limited Access",RestrictedRead:"Restricted Read",RestrictedContribute:"Restricted Contribute",InitialCreate:"Initial Create"},ya={RestrictedReaders:new K({ID:null,Title:"Restricted Readers"})},We={Admin:"Admin",ActionOffice:"ActionOffice"},Se={ActionResolver:{LookupValue:"Action Resolver",description:"Confirms completion of an action.",isAssignable:!0,permissions:X.RestrictedContribute,initialStatus:E.InProgress},Assigner:{LookupValue:"Assigner",description:"Can create additional assignments.",isAssignable:!0,permissions:X.RestrictedContribute,initialStatus:E.InProgress},Approver:{LookupValue:"Approver",description:"Approves or Rejects the request.",isAssignable:!0,permissions:X.RestrictedContribute,initialStatus:E.InProgress},Viewer:{LookupValue:"Viewer",description:"Has view only access to the request.",isAssignable:!0,permissions:X.RestrictedRead},Subscriber:{LookupValue:"Subscriber",description:"Has view only access to the request and recieves notifications",isAssignable:!0,permissions:X.RestrictedRead}},Rt={"Pending Assignment":Se.Assigner,"Pending Approval":Se.Approver,"Pending Action":Se.ActionResolver,"Pending Resolution":Se.ActionResolver,Notification:Se.Subscriber},F=ko.observable(),Ct=class s extends K{constructor({ID:e,Title:t,LoginName:i=null,LookupValue:o=null,WorkPhone:a=null,EMail:d=null,IsGroup:m=null,IsEnsured:b=!1,Groups:x=null,Department:O=null}){super({ID:e,Title:t,LookupValue:o,LoginName:i,IsGroup:m,IsEnsured:b}),this.WorkPhone=a,this.EMail=d,this.OfficeSymbol=O??"CGFS/EX",this.Groups=x}OfficeSymbol;Groups=[];isInGroup(e){return e?.ID?this.getGroupIds().includes(e.ID):!1}getGroupIds(){return this.Groups.map(e=>e.ID)}isInRequestOrg=e=>this.RequestOrgs().find(t=>t.ID==e.ID);RequestOrgs=ko.pureComputed(()=>{let e=this.getGroupIds();return ge().filter(t=>t.Everyone||e.includes(t.UserGroup?.ID))});RequestingOffices=ko.pureComputed(()=>this.RequestOrgs().filter(e=>e.OrgType==Kt.RequestingOffice));ActionOffices=ko.pureComputed(()=>this.RequestOrgs().filter(e=>e.OrgType==Kt.ActionOffice));IsActionOffice=ko.pureComputed(()=>this.ActionOffices().length);IsSiteOwner=ko.pureComputed(()=>this.isInGroup(Zt().owners));hasSystemRole=e=>{let t=this.IsSiteOwner();switch(e){case We.Admin:return t;case We.ActionOffice:return t||this.ActionOffices().length;default:}};static Create=async function(){let e=await Pi();return new s(e)}};function Oi(s){let e=Zt(),t=s.RequestorInfo.Requestor(),i=s.Author.Value(),o=s.RequestorInfo.Office(),a=[[new K(e.owners),X.FullControl],[ya.RestrictedReaders,X.RestrictedRead],[t,X.RestrictedContribute],[i,X.RestrictedContribute]];return o&&!o.BreakAccess&&a.push([o.UserGroup,X.RestrictedContribute]),s.Pipeline.Stages()?.forEach(d=>{let m=ee.FindInStore(d.RequestOrg);if(m&&a.push([m.UserGroup,X.RestrictedContribute]),d.AssignmentFunction&&ze[d.AssignmentFunction])try{ze[d.AssignmentFunction](s,d).forEach(x=>{let O=x.Assignee;O&&O.Title&&a.push([O,X.RestrictedContribute])})}catch{console.warn("Error creating stage assignments",d)}}),a}async function Li(s){let e=await Fi(s);return e?e.map(t=>new K(t)):[]}var ze={TestFunc:function(){return request.RequestorInfo.Requestor()},ch_overtimeGovManager:function(s,e){let t=s.RequestBodyBlob?.Value()?.GovManager.get();if(!t)throw new Error("Could not find stage Assignee");return[new de({Assignee:t,RequestOrg:e.RequestOrg,PipelineStage:e,IsActive:!0,Role:Se.ActionResolver,CustomComponent:"GovManagerActions"})]},ch_overtimeAPM:function(s,e){let t=s.RequestBodyBlob?.Value()?.FieldMap.APM.get();if(!t)throw new Error("Could not find stage Assignee");return[new de({Assignee:t,RequestOrg:e.RequestOrg,PipelineStage:e,IsActive:!0,Role:Se.ActionResolver,CustomComponent:"APMActions"})]},getGTM:function(s,e){let t=s.RequestBodyBlob?.Value()?.FieldMap.GTM.get();if(!t)throw new Error("Could not find stage Assignee");return[new de({Assignee:t,RequestOrg:e.RequestOrg,PipelineStage:e,IsActive:!0,Role:Se.Approver})]},getCOR:function(s,e){let t=s.RequestBodyBlob?.Value()?.FieldMap.COR.get();if(!t)throw new Error("Could not find stage Assignee");return[new de({Assignee:t,RequestOrg:e.RequestOrg,PipelineStage:e,IsActive:!0,Role:Se.Approver})]},getSupervisor:function(s,e){return[new de({Assignee:Ei(s,"Supervisor"),RequestOrg:e.RequestOrg,PipelineStage:e,IsActive:!0,Role:Se.Approver})]},getWildcard:function(s,e,t){return[new de({Assignee:Ei(s,t),RequestOrg:e.RequestOrg,PipelineStage:e,IsActive:!0,Role:Se.Approver,CustomComponent:e.ActionComponentName})]}};function Ei(s,e){let t=s.RequestBodyBlob?.Value()?.FieldMap[e]?.get();if(!t)throw new Error(`Could not find assignee field on current request: ${e}`);return t}var T=String.raw,M=class{constructor(){}static name="base-component";static template=T`<div>No Component Registered!</div>`};var Vi=T`
...
`,at=class extends Y{constructor(e){super(e)}static viewTemplate=Ca;static editTemplate=Aa;static view="checkbox-view";static edit="checkbox-edit";static new="checkbox-edit"};se(at);var Ra=H`
  ...
  <!-- /ko -->
  <!-- /ko -->
`,rt=class extends Y{constructor(e){super(e)}static editTemplate=Ra;static view="date-view";static edit="date-edit";static new="date-edit"};se(rt);var Ta=H`
  <label class="fw-semibold"
    ><span data-bind="text: displayName"></span
    ><span data-bind="if: isRequired" class="fw-bold text-danger">*</span>:
    <!-- ko if: isSearch -->
  ...
  <!-- /ko -->
  <!-- /ko -->
`,nt=class extends Y{constructor(e){super(e)}static editTemplate=Ta;static view="lookup-view";static edit="lookup-edit";static new="lookup-edit"};se(nt);var xa=H`
...
optionsText: 'Title'"
  ...
  ></div>
  <!-- /ko -->
`;var mi=class extends M{constructor(e){super(),this.key=e.key}ServiceTypes=ko.pureComputed(()=>ue().filter(e=>e.userCanInitiate(F())));SelectedService=ko.observable();static name="requests-by-service-type";static template=xo};var qo=T`
...
`;var $t=class extends M{constructor(e){super(),Object.assign(this,e)}AvailableRequestorOffices=ko.pureComputed(()=>F()?.RequestingOffices());AvailableServiceTypes=ko.pureComputed(()=>ue().filter(e=>e.userCanInitiate(F())))},gi=class extends $t{constructor(e){super(e)}static name="request-header-view";static template=Do},bi=class extends $t{constructor(e){super(e)}static name="request-header-edit";static template=Io};var Po=T`
...
`;var wi=class extends M{constructor({ShowActionOfficeFeatures:e,ToggleActionOfficeFeatures:t}){super(),this.ShowActionOfficeFeatures=e,this.ToggleActionOfficeFeatures=t}ShowActionOfficeToggle=ko.pureComputed(()=>F()?.IsActionOffice()&&!1);MyOpenAssignments=xe.MyActiveAssignments;LateRequests=ko.pureComputed(()=>ie.get(I.open)?.List()?.filter(e=>e.Dates.EstClosed.Value()<=new Date)??[]);toggleInfoContainer=(e,t)=>{t.target.closest(".status-info").classList.toggle("active")};static name="quick-info";static template=No};var Eo=T`
